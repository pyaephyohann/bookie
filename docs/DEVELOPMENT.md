...
## A10 learnings (banner management)

- **Reuse Hero Slides pattern:** Banner management follows the exact same architecture as Hero Slides (A7.1) — list page with search/filter/pagination, create/edit form with ImageField, server actions with requireAdmin(), revalidation. Copy the Hero Slides implementation and adapt fields rather than building from scratch.
- **BannerStatus vs isActive:** unlike Hero Slides which use `isActive` boolean, Banners use a `BannerStatus` enum (DRAFT/PUBLISHED/ARCHIVED). The publish/unpublish toggle switches between PUBLISHED and DRAFT. The `bannerStatus()` helper derives LIVE/SCHEDULED/EXPIRED/INACTIVE from the enum + scheduling dates.
- **Nullable title in schema:** the Prisma `Banner.title` field is `String?` (nullable), but the Zod schema requires it (`min(1)`). When passing edit data to the form, convert nullable fields with `?? ""` and cast status to the enum union type.
- **UploadFolder extension:** adding a new content type with image uploads requires adding the folder name to `UploadFolder` type and `FOLDER_MAP` in `lib/cloudinary.ts`. Use a descriptive folder name (e.g., `banners`).
