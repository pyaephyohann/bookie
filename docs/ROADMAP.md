...
| B9 | **Online Reading** | ✅ Completed — real reader at `/books/[slug]/read` with BookContent HTML/text rendering, PDF embed, inline EPUB rendering via epubjs (paginated flow, dark/light theme, font-size integration), OTHER file download card, font size + reading width controls, scroll progress, localStorage reading position, distraction-free chrome, unavailable states, B3 entry point wired |
...
| A8 | **Admin Settings & User Management** | 🔒 Locked — Hero Slides added to admin sidebar navigation; admin/staff user management (list, create, edit, activate/deactivate, password reset) with ADMIN-only authorization, self-deactivation prevention, last-active-admin lockout prevention; settings page updated with user management link |
| A9 | **Production Polish** | 🔒 Locked — Footer/mobile navigation dead-link fixes, design-system dev page removal, legacy CartContext cleanup, checkout label accessibility, admin focus-style standardization, Cmd+K search mock-removal and navigation launcher, checkout success-color semantic tokens, SearchCommand ARIA fix, admin settings placeholder cleanup, TypeScript type fix |
| A10 | **Banner Management** | ✅ A10.1 Admin CRUD — Banner list/create/edit/delete with search/status filter, image upload via Cloudinary, publish/unpublish toggle, reorder, scheduling (startAt/endAt), ADMIN-only authorization, homepage revalidation; A10.2 Storefront integration — pending |
