...
| A7 | **Content & Promotions** | 🔒 Locked — A7.1 Hero Slides (database-driven, admin CRUD with Cloudinary image uploads, scheduling, sort order, active/inactive toggle, homepage integration with mock fallback); reading content management (inline HTML/text with sanitize-html, PDF/EPUB/OTHER file URLs, isReadableOnline toggle); supported featured shelves (Trending / Best Sellers / Recommended) with assign/remove/reorder; promotions CRUD with activation/scheduling/book linking |
