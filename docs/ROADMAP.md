...
| A7 | **Content & Promotions** | 🔒 Locked — A7.1 Hero Slides (database-driven, admin CRUD with Cloudinary image uploads, scheduling, sort order, active/inactive toggle, homepage integration with mock fallback); A7.2 Staff Picks (FeaturedBook section TRENDING/BEST_SELLER/STAFF_PICK/RECOMMENDED managed at /admin/content/featured with assign/remove/reorder, homepage integration with deterministic fallback); reading content management (inline HTML/text with sanitize-html, PDF/EPUB/OTHER file URLs, isReadableOnline toggle); promotions CRUD with activation/scheduling/book linking |
